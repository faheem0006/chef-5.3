...
class StoreController extends CController
{
	...
	public function actionHome()
	{
		...
		if ($map['provider'] == "mapbox") {
			...
			$cs->registerScriptFile(
				$baseUrl . "/assets/vendor/leaflet/plugin/geocoder/mapbox-gl-geocoder.min.js",
	...
	public function actionSearchArea()
	{
		...
		if (isset($_GET['foodname'])) {
			$_SESSION['kr_search_foodname'] = 'all';
			$_SESSION['search_type'] = 'kr_search_foodname';
		}

		if (isset($_GET['mealplans'])) {
			$_SESSION['kr_search_meal'] = 'all';
			$_SESSION['search_type'] = 'kr_search_meal';
		}

		if (isset($_GET['dailyspecial'])) {
			$_SESSION['kr_search_daily'] = 'all';
	...
	public function actionMenu()
	{
		...
		if ($map['provider'] == "mapbox") {
			...
			$cs->registerScriptFile(
				$baseUrl . "/assets/vendor/leaflet/plugin/routing/leaflet-routing-machine.min.js",
				CClientScript::POS_END
			);
		...
		//dump($page_slug);
		if (isset($data['merchant'])) { } else $data['merchant'] = $page_slug;
					...
					switch ($res['transaction_type']) {
						...
						case "signup":
							...
							if ($res['transaction_type'] == "renew") {
								...
								$membership_info = Yii::app()->functions->upgradeMembership(
									$token_details['merchant_id'],
									$package_id
								);
	...
	public function actionGoogleLogin()
	{
		...
		try {
			if (
				!isset(Yii::app()->session['auth_token'])
				...
				if (is_array($t) && count($t) >= 1) {
					...
					if ($resp_t = $func->googleRegister($t)) {
						...
						if (isset($resp_t['verification_type'])) {
							...
							Yii::app()->functions->clientAutoLogin(
								$t['email'],
								$resp_t['password'],
								$resp_t['password']
							);
	...
	public function actionvoginit()
	{
		...
		if ($data = Yii::app()->functions->getOrder($_GET['id'])) {
			...
			$amount_to_pay = $data['total_w_tax'];
			$error = '';
				...
				if ($order_info = FunctionsV3::getOrderInfoByToken($reference_id)) {
					...
					if ($credentials = mercadopagoWrapper::getCredentials($merchant_id)) {
						try {
							...
							FunctionsV3::updateOrderPayment(
								$order_id,
								mercadopagoWrapper::paymentCode(),
								$merchant_order_id,
								$data,
								$reference_id
							);
	...
	public function actionBecomechef()
	{
		$this->render('become_chef');
	}
