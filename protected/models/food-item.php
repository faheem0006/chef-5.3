...
if ($this->data['tbl'] == 'viewDailySpecial') {
  $data = Yii::app()->functions->getDailySpecialById($this->data['item_id']);
} else {
...
<?php if (is_array($data) && count($data) >= 1) : ?>
  ...
  <form class="frm-fooditem" id="frm-fooditem" method="POST" onsubmit="return false;">
    ...
    <?php if ($this->data['tbl'] == 'viewDailySpecial') : ?>
    <?php echo CHtml::hiddenField('dsFlag', 1) ?>
    <?php endif; ?>
   ...
   if(!is_array($data['prices']) && empty($data['prices'])){
      ...
      <?php if ($disabled_website_ordering == "") : ?>
        ...
        <div class="row food-item-actions">
          ...
          <div class="col-md-4 col-xs-4 border into-row">
            <input type="submit" value="<?php echo empty($row) ? Yii::t("default", "add to cart") : Yii::t("default", "update cart"); ?>" class="add_to_cart orange-button upper-text">
          </div>
          <div class="col-md-4 col-xs-4 border into-row">
