...
$list = Yii::app()->functions->getAddOnLists($merchant_id);
?>
...
<form class="uk-form uk-form-horizontal forms" id="forms">
  ...
  <?php echo CHtml::hiddenField('table', 'subcategory') ?>
  <?php echo CHtml::hiddenField('whereid', 'subcat_id') ?>
    ...
    <ul class="uk-sortable" data-uk-sortable>
      <?php foreach ($list as $val) : ?>
        <li class="uk-panel uk-panel-box" style="list-style:none;margin-bottom:5px;">
          <?php echo CHtml::hiddenField('sort_field[]', $val['subcat_id']) ?>
          <i class="fa fa-arrows-alt"></i>
          <?php echo $val['subcategory_name'] ?>
        </li>
      <?php endforeach; ?>
    </ul>
  <?php else : ?>
