...
class FunctionsV3
{
	...
	public static function getMealPlan($plan_id='')
	{
		$q = FunctionsV3::q($plan_id);
		$DbExt = new DbExt;
		$stmt = "
		SELECT *
		FROM 
	...
	public static function getMenu($class = "menu")
	{
		...
		$become_a_chef_link = "/store/becomechef";
		...
		if (getOptionA('merchant_disabled_registration') != "yes") {
			$top_menu[] = array(
				...
				'label' => '' . Yii::t("default", "Become a Chef"),
				'url' => array($become_a_chef_link)
			);
		}
	...
	*/
	...
	public static function getMerchantByName($merchant_id = '')
	{
		$q = FunctionsV3::q($merchant_id);
		$DbExt = new DbExt;
		$stmt = "
			...
			if (is_object($data)) {
				if ($data->status == "OK") {
					...
					if ($units_params == "imperial") {
						...
						if (preg_match("/km/i", $distance_value)) { } else {
	...
	public static function getSlugByMerchant($merchant_id = '')
	{
		/*$p = new CHtmlPurifier();
		$slug_id = $p->purify($slug_id);*/

		$DbExt = new DbExt;
		$stmt = "SELECT restaurant_slug FROM mt_merchant WHERE merchant_id=" . q($merchant_id) . " LIMIT 0,1";
		$DbExt->qry("SET SQL_BIG_SELECTS=1");
		if ($res = $DbExt->rst($stmt)) {
			return $res[0];
		}
	...
	public static function sectionHeader($title = '')
	{
		...
		";
												...
												public static function getLanguageList($aslist = true)
												{
													...
													if (is_array($res) && count($res) >= 1) {
														foreach ($res as $val) {
															if ($val == ".") { } elseif ($val == "..") { } elseif ($val == "default") { } else {
																$list[$val] = $val;
															}
														}
														return $list;
												...
												public static function sendWelcomeEmailMerchant($data = '', $force_send = false)
												{
													...
													$disabled_verification = getOptionA("merchant_email_verification");
													if ($disabled_verification == "yes") { } else {
														if ($force_send == false) {
															return;
														}
													}
													...
													foreach ($pattern as $key => $val) {
														switch ($key) {
															...
															case "total_amount":
																$tpl = self::smarty(
																	$key,
																	isset($data[$val]) ? FunctionsV3::prettyPrice($data[$val]) : '',
																	$tpl
																);
																break;
       ...
       a.area_id = e.area_id
		...
		";
												...
												public function getAddonItemList($merchant_id = '')
												{
		...
		";
												...
												public static function getChefName($merchant_id = '')
												{
													$DbExt = new DbExt;
													$stmt = "SELECT contact_name FROM
			mt_merchant
			WHERE
			merchant_id=" . self::q($merchant_id) . "
		";
												...
												public static function getAddonItemCount($merchant_id = '')
												{
													$DbExt = new DbExt;
		...
		";
												...
												public static function getMerchantDishes($merchant_id = '')
												{
													//$q= "'". '%"'.$size_id.'":%' ."'";
													$q = FunctionsV3::q($merchant_id);
													$DbExt = new DbExt;
													$stmt = "SELECT COUNT(*) FROM {{item}} WHERE merchant_id = " . ($q) . " LIMIT 0,1";
													if ($res = $DbExt->rst($stmt)) {
														return $res;
													}
													return false;
												...
												public static function getItemCategory($category_id = '')
												{
													$q = FunctionsV3::q($category_id);
													$DbExt = new DbExt;
													$stmt = "SELECT category_name FROM {{category}} WHERE category_id = " . ($q) . " LIMIT 0,1";
													if ($res = $DbExt->rst($stmt)) {
														return $res;
													}
													return false;
    	...
    	die();*/
													...
													if (is_array($times) && count($times) >= 1) { } else {
