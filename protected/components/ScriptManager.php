...
class ScriptManager
{
	public static function RegisterAllJSFile($config = array())
	{
		...
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/DataTables/jquery.dataTables.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/DataTables/fnReloadAjax.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/JQV/form-validator/jquery.form-validator.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/jquery.ui.timepicker-0.0.8.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/SimpleAjaxUploader.min.js",
			CClientScript::POS_END
		);


		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/iCheck/icheck.js",
			CClientScript::POS_END
		);
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/chosen/chosen.jquery.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/markercluster.js",
			CClientScript::POS_END
		);

		if (!empty($config['google_key'])) {
			$cs->registerScriptFile(
				"//maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&key=" . $config['google_key'],
				CClientScript::POS_END
			);
		} else {
			$cs->registerScriptFile(
		...
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/jquery.geocomplete.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/jquery.printelement.js",
			CClientScript::POS_END
		);
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/fancybox/source/jquery.fancybox.js?ver=1",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/jquery.appear.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/magnific-popup/jquery.magnific-popup.js",
			CClientScript::POS_END
		);


		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/imagesloaded.pkgd.js",
			CClientScript::POS_END
		);
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/intel/build/js/intlTelInput.js?ver=2.1.5",
			CClientScript::POS_END
		);
		...
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/bootstrap/js/bootstrap.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/parallax.js/parallax.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/raty/jquery.raty.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			"//cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.9.4/js/bootstrap-select.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/waypoints/jquery.waypoints.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/waypoints/shortcuts/infinite.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/gmaps.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/noty-2.3.7/js/noty/packaged/jquery.noty.packaged.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/nprogress/nprogress.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/theia-sticky-sidebar.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/readmore.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/justified-gallery/js/jquery.justifiedGallery.min.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/EasyAutocomplete/jquery.easy-autocomplete.min.js",
			CClientScript::POS_END
		);

		if ($config['theme_time_pick'] == 2 || $config['website_use_time_picker'] == 2) {
			$cs->registerScriptFile(
				$baseUrl . "/assets/vendor/timepicker.co/jquery.timepicker.js",
				CClientScript::POS_END
			);
		}

		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/typehead/bootstrap3-typeahead.min.js",
			CClientScript::POS_END
		);
		...
		if ($config['map_provider'] == "mapbox") {
			$cs->registerScriptFile(
				$baseUrl . "/assets/vendor/leaflet/leaflet.js",
				CClientScript::POS_END
			);
		}
		/*LEAFLET*/
		...
		$cs->registerScriptFile(
			$baseUrl . "/assets/vendor/jquery-cookie.js",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/js/store.js?ver=3",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/js/store-v3.js?ver=3",
			CClientScript::POS_END
		);

		$cs->registerScriptFile(
			$baseUrl . "/assets/js/k_mapbox.js?ver=1.0",
			CClientScript::POS_END
		);
