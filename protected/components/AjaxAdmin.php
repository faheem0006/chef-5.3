	...
	class AjaxAdmin extends DbExt
	{
		...
		public function addOnItemNew()
		{
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				if ($res) {
					$this->code = 1;
					$this->msg = Yii::t("default", 'Meal Plan Dish updated.');
				...
				if ($res = $command->insert('{{subcategory_item}}', $params)) {
					...
					$this->code = 1;
					$this->msg = Yii::t("default", 'Meal Plan Dish added.');
					$this->details = $item_id;
					// Redirect to Food Item List!
				} else $this->msg = Yii::t("default", 'ERROR. cannot insert data.');
			}
		}

		public function addOnMealPlan()
		{
			$sql = "INSERT INTO mt_meal_plan (mo, tu, we, th, fr, merchant_id, title, is_expired, avail, mo_date, tu_date, we_date, th_date, fr_date) 
			VALUES 
			(:m, :t, :w, :t, :f, :mid, :title, :ie, :avail, :mo_d, :tu_d, :we_d, :th_d, :fr_d)";

			$parameters = array(
				':m' => addslashes($this->data['meal_days_Monday']),
				':t' => addslashes($this->data['meal_days_Tuesday']),
				':w' => addslashes($this->data['meal_days_Wednesday']),
				':t' => addslashes($this->data['meal_days_Thursday']),
				':f' => addslashes($this->data['meal_days_Friday']),
				':mid' => Yii::app()->functions->getMerchantID(),
				':title' => addslashes($this->data['title']),
				':ie' => 1,
				':avail' => addslashes($this->data['avail']),
				':mo_d' => addslashes($this->data['meal_day_Monday']),
				':tu_d' => addslashes($this->data['meal_day_Tuesday']),
				':we_d' => addslashes($this->data['meal_day_Wednesday']),
				':th_d' => addslashes($this->data['meal_day_Thursday']),
				':fr_d' => addslashes($this->data['meal_day_Friday'])
			);

			$command = Yii::app()->db->createCommand($sql);

			if ($res = $command->execute($parameters)) {
				$item_id = Yii::app()->db->getLastInsertID();
				$this->code = 1;
				$this->msg = Yii::t("default", 'Plan Added!');
				$this->details = $item_id;
			} else {
				$this->msg = Yii::t("default", 'ERROR. Cannot Add Meal Plan.');
			}
		}
		...
		public function FoodItemAdd()
		{
			...
			if (!Yii::app()->functions->validateMerchantCanPost($mtid)) {
				if (isset($this->data['id']) && is_numeric($this->data['id'])) { } else {
			...
			$params = array(
				...
				'item_mt' => isset($this->data['item_mt']) ? $this->data['item_mt'] : 0,
				'item_serve' => isset($this->data['item_serve']) ? $this->data['item_serve'] : 0,
				'item_quantity' => isset($this->data['item_quantity']) ? $this->data['item_quantity'] : 0,
				'item_order_time' => isset($this->data['item_order_time']) ? $this->data['item_order_time'] : 0
			);
		...
		// Add Daily Special
		public function DailySpecialAdd()
		{
			$mtid = isset($_GET['chefid']) ? $_GET['chefid'] : 1;
			if (!Yii::app()->functions->validateMerchantCanPost($mtid)) {
				if (isset($this->data['id']) && is_numeric($this->data['id'])) { } else {
					$this->msg = Yii::t("default", "Sorry but you reach the limit of adding food item. Please upgrade your membership");
					return;
			...
			if (isset($this->data['id']) && is_numeric($this->data['id'])) {
				...
				if ($res = $command->insert('{{daily_special}}', $params)) {
					$this->details = Yii::app()->db->getLastInsertID();
					$this->code = 1;
					$this->msg = Yii::t("default", 'Item added.');
				} else $this->msg = Yii::t("default", 'ERROR. cannot insert data.');
		...
		public function viewDailySpecial()
		{
			if (isset($this->data['item_id'])) {
				require_once 'ds-item.php';
			} else {
				?>
				...
				<?php
						...
						public function addToCart()
						{
							if( isset($this->data['dsFlag']) ){
								...
								if ($website_disabled_cart_validation != 2) :
									...
									if ($food_info = Yii::app()->functions->getDailySpecialItem($h_item_id)) {
						...
						dump("h_discount=>$h_discount");*/
								...
								if (isset($this->data['item_id'])) {
									...
									if ($food_info = Yii::app()->functions->getDailySpecialItem($item['item_id'])) {
	    		    ...
	    		    dump("h_discount=>$h_discount");*/
						...
						// Daily Special ADD TO Cart
						public function addToCartDailySpecial()
						{
...
$params['cart_tip_value']=isset($this->data['cart_tip_value'])?$this->data['cart_tip_value']:''; */
			            	...
			            	if ( Yii::app()->functions->getOption("merchant_packaging_increment",$mtid)==2){
		                        ...
		                        $params['packaging']=$total_cart_item*$merchant_packaging_charge;
		    		        		...
		    		        		$this->data['map_address_lng']);*/
													...
													$geo_res = FunctionsV3::latToAdress(
														$this->data['map_address_lat'],
														$this->data['map_address_lng']
													);
													if ($geo_res) {
	    		        ...
	    		        die();*/
						     		...
						     		";
											...
											/*VOUCHER*/
											if ($has_voucher == TRUE) {
												Yii::app()->functions->updateVoucher(
													$_SESSION['voucher_code']['voucher_name'],
													Yii::app()->functions->getClientId(),
													$order_id
												);
											}
									...
									if ($res = yii::app()->functions->isClientExist($this->data['username-email'])) {
										...
										if ($this->updateData("{{client}}", $params, 'client_id', $res['client_id'])) {
											...
											if ($enabled) {
												...
												if (!empty($subject)) {
													$subject = FunctionsV3::smarty(
														'firstname',
														isset($res['first_name']) ? $res['first_name'] : '',
														$subject
													);

													$subject = FunctionsV3::smarty(
														'lastname',
														isset($res['last_name']) ? $res['last_name'] : '',
														$subject
													);
												}
		    ...
		    */
									...
									if ($res = Yii::app()->functions->getVoucherCodeNew($this->data['voucher_code'], $this->data['merchant_id'])) {
										...
										if ($res = Yii::app()->functions->getVoucherCodeAdmin($this->data['voucher_code'])) {
											...
											if (!empty($res['joining_merchant'])) {
												...
												if (in_array($this->data['merchant_id'], (array) $joining_merchant)) { } else {
			...
			";
    	    		...
    	    		<a href=\"$reply_link\" >" . Yii::t("default", "Reply") . "</a>
										...
										if ($this->data['currentController'] == "admin") { } else {
