...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	public function DailySpecialAdd()
		{
			...
			// Remove Check of Max Food Item Add for Admin
			...
			$command = Yii::app()->db->createCommand();
			if ($res = $command->insert('mt_daily_special', $params)) {
				$this->details = Yii::app()->db->getLastInsertID();
				$this->code = 1;
				$this->msg = Yii::t("default", 'Daily Special Added!');
			} else $this->msg = Yii::t("default", 'ERROR. cannot create Daily Special.');
		}
	...
	public function merchantList()
	{
		...
		$theUrl = Yii::app()->request->baseUrl;
		$slug=$this->data['slug'];
		...
		if ( $res=$this->rst($stmt)){		
			...
			foreach ($res as $val) {	
					...
					<a href=\"$theUrl/merchant/add-daily-special?chefid=$val[merchant_id]\" >".Yii::t("default","Daily Special")."</a>
