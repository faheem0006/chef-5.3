			...
			if (data.code == 1) {
				...
				if (action == "placeOrder") {
					...
					switch (data.details.payment_type) {
						...
						case "obd":
						case "pyr":
							window.location.replace(sites_url + "/profile?vieworders");
							break;
...
jQuery(document).ready(function () {
	...
	$(document).on('click', '.btn-add-dish-from-signup', function () {
		...
		$.ajax({
			type: "POST",
			url: ajax_url,
			data: "action=addDishAfterMerchantSignUp&currentController=store&" + params,
			dataType: 'json',
			success: function (data) {
				busy(false);
				if (data.code == 1) {
	...
	// Daily Special 
	$(document).on("click", ".ds-item", function () {
		...
		if (single == 2) {
			if ($("#website_disbaled_auto_cart").val() != "yes") {
				...
				single_daily_item_add(id, $(this).data("price"), $(this).data("size"), $(this).data("category_id"), $(this).data("size_id"), $(this).data("discount"));
				return;
			}
		}
		...
		var params = "action=viewFoodItem&currentController=store&item_id=" + id + "&tbl=viewDailySpecial";
		params += "&category_id=" + $(this).data("category_id");
		open_fancy_box(params);
	});
	// End Daily Special
	...
	$(document).on("click", ".place_order", function () {
		...
		if (typeof transaction_type === "undefined" || transaction_type == null || transaction_type == "" || transaction_type == "null" || transaction_type == "undefined") {} else {
...
// Daily Special
function single_daily_item_add(item_id, price, size, category_id, size_id, discount) {
	var params = '';
	params = "merchant_id=" + $("#merchant_id").val();
	params += "&item_id=" + item_id
	if (size == "") {
	...
	$.ajax({
		type: "POST",
		url: ajax_url,
		data: "action=addToCartDailySpecial&currentController=store&" + params,
		dataType: 'json',
		success: function (data) {
			busy(false);
			if (data.code == 1) {
